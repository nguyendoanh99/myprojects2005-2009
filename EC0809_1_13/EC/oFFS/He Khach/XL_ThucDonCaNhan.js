...
function XL_ThucDonCaNhan()
{
}

var DsMaMonAn = new Array();

function XL_ThucDonCaNhan.Lay_ma_thuc_don()
{
    var ham = new XL_HAM("He phuc vu/XL_ThucDonCaNhan");
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("xac_nhan", "Lay_ma_thuc_don"));
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("t", new Date().getTime()));
    var goc = ham.Thuc_hien();
    if(goc != null)
        return goc.getAttribute("Ma_thuc_don");
    return null;
}

function XL_ThucDonCaNhan.LoadDSMonChonTuThucDonCoSan()
{
   var mathucdon = XL_ThucDonCaNhan.Lay_ma_thuc_don();
   if(mathucdon == "-1")
        return;
   var dsMonAn = XL_ThucDon.ChiTietThucDon(mathucdon); // array obj MonAn   
   
   for(var i = 0; i < dsMonAn.length; ++i)
   {
        var obj = document.createElement("myobj");
        obj.MaMon = dsMonAn[i].MaMon;
        obj.TenMon = dsMonAn[i].TenMon;
        obj.HinhAnh = dsMonAn[i].HinhAnhMinhHoa;
        obj.Gia = dsMonAn[i].Gia;
        XL_ThucDonCaNhan.ChonMonAn(obj);
   }
}
function XL_ThucDonCaNhan.HienThiTrangChu()
{
    var ham = new XL_HAM("He phuc vu/XL_ThucDonCaNhan");
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("xac_nhan", "Lay_danh_sach_thuc_don_tu_tao"));    
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("t", new Date().getTime()));
    ...
    if(goc != null)
    {
        ...
        var th = document.getElementById("Th_thuc_don_ca_nhan");
        //XL_THUC_DON.XL_ThucDonCaNhan(arr, th);
        XL_ThucDonCaNhan.Th_Danh_sach_thuc_don(arr, th);
    }
}


function XL_ThucDonCaNhan.PhanTrang()
{
    ...
    if(goc != null)
    {
        ...
        for (var i = 1; i <= tongSoTrang; ++i)
        {
            ...
            aTag.innerText = i;
            aTag.href="javascript:XL_ThucDonCaNhan.LayDSMonAnHienThi(" + i + ")";
            td.appendChild(aTag);            
            tr.appendChild(td);
        }
...
function XL_ThucDonCaNhan.LayDSMonAnHienThi(pageNum)
{
    var ham = new XL_HAM("He phuc vu/XL_Mon_An");
    ...
    if(goc != null)
    {
        ...
        {   
            ...
            but.type = "button";
            but.onclick= function() {XL_ThucDonCaNhan.ChonMonAn(this);};
            but.value = "Chọn";
            but.MaMon = mamon;
            but.TenMon = tenmon;
            but.HinhAnh = hinhanh;
...
function XL_ThucDonCaNhan.ChonMonAn(obj)
{
    var mamon = obj.MaMon;
...
//    }   
    ...
    var but = document.createElement("input");
    but.type = "button";
    but.onclick= function() {XL_ThucDonCaNhan.BoMonAn(this);};
    but.value = "Bỏ";
    but.MaMon = mamon;
    but.Gia = gia;
    but.id = "butBo" + mamon;        
...
function XL_ThucDonCaNhan.BoMonAn(obj)
{
    var mamon = obj.MaMon;
    var gia = obj.Gia;
    var tbody = document.getElementById("tbodyDSMonAnChon");
...
function XL_ThucDonCaNhan.ThemThucDonMoi()
{
    ...
    var ham = new XL_HAM("He Phuc Vu/XL_ThucDonCaNhan");
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("xac_nhan", "ThemThucDonTuTao"));
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("t", new Date().getTime()));
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("ten", ten));
    ham.Danh_sach_tham_so.push(new XL_THAM_SO("hinh_anh", hinh_anh));
...
function XL_ThucDonCaNhan.KhoiTao()
{
...
function XL_ThucDonCaNhan.Ghi_nhan_thuc_don()
{
    var obj = window.event.srcElement.parentNode.parentNode;      
    var parent = obj.previousSibling;    
    parent = parent.previousSibling.previousSibling.previousSibling;
    ...
    Tham_so = new XL_THAM_SO("Ma_thuc_don", ma);
    Ham.Danh_sach_tham_so.push(Tham_so);

    var Goc = Ham.Thuc_hien();
}

function XL_ThucDonCaNhan.Chi_tiet_thuc_don()
{
    var Kq = null;
    var Ham = new XL_HAM("He Phuc Vu/XL_ThucDonCaNhan");
    ...
    if(Goc!=null)
    {
        Th = document.getElementById("Th_Chi_tiet_thuc_don_ca_nhan");
...
function XL_ThucDonCaNhan.Ghi_nhan_thuc_don1(mathucdon)
{
    var Kq = null;
    var Ham = new XL_HAM("He Phuc Vu/XL_ThucDonCaNhan");
    var Tham_so = new XL_THAM_SO("Xac_nhan","Ghi_nhan_thuc_don");
    Ham.Danh_sach_tham_so.push(Tham_so);
    Tham_so = new XL_THAM_SO("Ma_thuc_don", mathucdon);
    Ham.Danh_sach_tham_so.push(Tham_so);

    var Goc = Ham.Thuc_hien();
    
    window.location = "ChiTietThucDonCaNhan.aspx";
}

function XL_ThucDonCaNhan.Th_Danh_sach_thuc_don(arr, Th)
{
    var temp;
    var m;
    ...
    h1.align = "center";
    h1.appendChild(document.createTextNode("Danh Sách Thực Đơn Cá Nhân"));    
    td.appendChild(h1);
	...
	{	
       ...
       // if(curRow<low)
        ...
        var a = document.createElement("a");
        a.href = "javascript:XL_ThucDonCaNhan.Ghi_nhan_thuc_don1("+thuc_don.MaThucDon+")";
        ...
        input.name = "Order";        
        input.value = "Order";
        input.onclick =  XL_GIO_HANG.Ghi_nhan_dat_hang_thuc_don_ca_nhan ;
        div.appendChild(input);
