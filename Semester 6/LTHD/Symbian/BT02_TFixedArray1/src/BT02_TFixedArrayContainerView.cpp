...
void CBT02_TFixedArrayContainerView::HandleCommandL( TInt aCommand )
	...
	switch ( aCommand )
		...
		case EBT02_TFixedArrayContainerView_10_doi_tuong_TDiemMenuItemCommand:
			commandHandled = Handle_10_doi_tuong_TDiemMenuItemSelectedL( aCommand );
			break;
		default:
			break;
		}
 ...
 */
TBool CBT02_TFixedArrayContainerView::Handle_10_doi_tuong_TDiemMenuItemSelectedL( TInt aCommand )
	{
	...
	TBuf<100> buf;
	TPtr chuoi = temp.Des();
    buf.Format( _L("TDiem(%d, %d)"), MangDiem[0].DiemX(), MangDiem[0].DiemY());
	chuoi.Append(buf);
