...
public class TRUONGDAO extends AbstractDAO{
	...
	public boolean ThemKhoi(TRUONGDTO truongdto, KHOIDTO khoidto)
	{
		boolean kq = true;
		
		Element TruongElement = doc.getDocumentElement();
		Element DanhSachElement = (Element) TruongElement.getElementsByTagName("DanhSachKhoi").item(0);
		// Khong tim thay
		if (DanhSachElement == null)
		{			
			DanhSachElement = doc.createElement("DanhSachKhoi");
			TruongElement.appendChild(DanhSachElement);
		}
		...
		// Them vao dto
		truongdto.getDanhSachKhoiHoc().add(khoidto);
		return kq;
	}
}