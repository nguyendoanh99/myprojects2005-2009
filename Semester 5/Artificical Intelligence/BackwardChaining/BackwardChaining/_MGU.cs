...
{    
    class _MGU
    {
        ...
        #endregion
        #region CompareTo
        public Boolean CompareTo(_MGU p)
        {
            if (p.Count != Count)
                return false;
            ...
            for (int i = 0; i < mContent.Count; ++i)
            {
                _Element t1 = (_Element) mContent[i];
                _Element t2 = (_Element) p.mContent[i];
                if (t1.CompareTo(t2) == false)
                    return false;
            }
            return true;
        }
        #endregion
        #endregion
        #region Khai bao toan tu
        #region operator+
        public static _MGU operator +(_MGU p, _MGU q)
        {
            _MGU result = new _MGU();
            
            for (int i = 0; i < p.Count; ++i)
                result.Add(new _Element(p[i]));

            for (int i = 0; i < q.Count; ++i)
            {
                Boolean flag = false;
                for (int j = 0; j < result.Count; ++j)
                    if (result[j].CompareTo(q[i]))
                    {
                        flag = true;
                        break;
                    }
