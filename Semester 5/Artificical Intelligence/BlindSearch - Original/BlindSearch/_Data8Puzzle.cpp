...
using namespace std;
_Data8Puzzle::_Data8Puzzle(char *filename)
{
	fstream f(filename);
...
{
//	return 0;
	// Bien doi Goal de tinh khoang cach Mahattan
	...
	for (int i = 8; i >= 0; --i)
	{
		t = temp % 10;
		if (t != 9)
			A[t / 3][t % 3] = i; // Cho biet t nam tai vi tri nao trong ma tran
		temp /= 10;
	}
	...
	for (int i = 8; i >= 0; --i)
	{
		...
		if (t != 9)
		{
			temp = A[t / 3][t % 3]; // Cho biet t nam tai vi tri nao
